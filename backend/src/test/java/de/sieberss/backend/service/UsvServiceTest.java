...
class UsvServiceTest {
    ...
    @Test
    void getUsvList_shouldReturnEmptyList_Response_Response_whenRepoIsEmpty() {
        List<Usv> expected = Collections.emptyList();
        when(repo.findAll()).thenReturn(expected);
        List<UsvResponseObject> actual = service.getUsvListResponse().usvs();
        assertEquals(expected, actual);
        verify(repo).findAll();
    }

    @Test
    void getUsvList_shouldReturnListOfUsvResponseObjects_whenRepoIsFilledResponse() {
        Usv usv = new Usv("1", "Test-USV", "192.168.1.1", "");
        List<UsvResponseObject> expected = List.of(new UsvResponseObject(usv, null));
        when(repo.findAll()).thenReturn(List.of(usv));
        List<UsvResponseObject> actual = service.getUsvListResponse().usvs();
        assertEquals(expected, actual);
        verify(repo).findAll();
    }
